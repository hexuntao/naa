...
export abstract class BaseRestful {
  ...
  protected async createRoutes() {
    ...
    this._routes = (
      await Promise.all(
        versionMaps.map(async ([name, version]) =>
          (
            await createRouteModuleTree(this.configure, this._modules, version.routes ?? [], name)
          ).map((route) => ({
            ...route,
    ...
    this._routes = [
      ...this._routes,
      ...(
        await createRouteModuleTree(this.configure, this._modules, defaultVersion.routes ?? [])
      ).map((route) => ({
        ...route,
