...
export class Restful extends BaseRestful {
  ...
  protected filterExcludeModules(routeModules: Type<any>[]) {
    ...
    return routeModules.filter((rmodule) => !excludeModules.find((emodule) => emodule === rmodule));
  }
  ...
  protected getRouteDocs(
    ...
    const hasAdditional = (doc?: ApiDocSource) => doc && Object.keys(omit(doc, 'tags')).length > 0;
