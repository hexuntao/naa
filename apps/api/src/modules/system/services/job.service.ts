...
@Injectable()
export class JobService extends BaseService<JobEntity, JobRepository> {
   ...
   * 查询树
   */
  async findTrees(options: QueryJobTreeDto) {
   ...
   * @param data
   */
  async update(data: UpdateJobDto) {
  ...
  protected async getParent(current?: string, parentId?: string) {
    ...
    let parent: JobEntity | undefined;
    if (parentId !== undefined) {
      if (parentId === null) return null;
      parent = await this.repository.findOne({ where: { id: parentId } });
      if (!parent) throw new EntityNotFoundError(JobEntity, `Parent id ${parentId} not exists!`);
    }
    return parent;
  }
}
