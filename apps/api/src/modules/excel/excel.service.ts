...
export class ExcelService {
   ...
   * @param options 配置
   */
  async create<Model, TModel extends Type<Model>>(
    model: TModel,
    rows: Model[],
  ...
  async import<TModel>(model: Type<TModel>, buffer: ExcelJS.Buffer, options?: ExportOptions) {
    ...
    const rows = worksheet.getSheetValues() as unknown as (number | string | any)[];
  ...
  private transformImage(
    row: ExcelJS.Row,
    cell: ExcelJS.Cell,
    rawCell: ExcelColumnOption,
  ): SheetImage {
    const { value } = cell;
    if (!value || !isString(value) || !/^http(s)?:\/\//.test(value))
      return { tl: undefined, ext: undefined, url: undefined, hyperlinks: undefined };
    cell.value = null;
