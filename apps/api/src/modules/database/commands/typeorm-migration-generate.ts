...
export class TypeormMigrationGenerate {
  async handler(args: HandlerOptions) {
    ...
    const directory = args.dir.startsWith('/') ? args.dir : path.resolve(process.cwd(), args.dir);
    ...
    try {
      ...
      if (args.dryrun) {
        console.log(
          chalk.green(
            `Migration ${chalk.blue(fullPath + extension)} has content:\n\n${chalk.white(
              fileContent,
            )}`,
          ),
        );
        ...
        console.log(
          chalk.green(`Migration ${chalk.blue(fullPath)} has been generated successfully.`),
        );
      }
    } catch (err) {
