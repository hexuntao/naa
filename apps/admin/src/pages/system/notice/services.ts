...
export function getPageList(params?: ListNoticeParams) {
  return httpRequest.get<PageResponse<NoticeModel>>('/notices', params);
}
...
export function getDetail(noticeId: React.Key) {
  return httpRequest.get<NoticeModel>(`/notices/${noticeId}`);
}
...
export function add(params: CreateNoticeParams) {
  return httpRequest.post('/notices', params);
}

/**
 * 更新
 */
export function update(noticeId: number, params: UpdateNoticeParams) {
  return httpRequest.put(`/notices/${noticeId}`, params);
}
...
export function deletes(noticeIds: React.Key) {
  return httpRequest.delete(`/notices/${noticeIds}`);
}
