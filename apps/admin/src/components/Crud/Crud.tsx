import { formatResponse, isSuccess } from './helpers';
import {
  ActionType,
  BetaSchemaForm,
  ParamsType,
...
>(
  ...
  const onDel = async (record: DataType) => {
    ...
    const { code, message: msg } = await api.delete([record[rowKeyName]]).finally(() => {
  ...
  const onSubmit = async (values: DataType) => {
    ...
    const { code, message: msg } = await (params[rowKeyName] ? api.update : api.create)(
      params,
    ).finally(() => {
      tableRef.current?.reload();
    });
  ...
  return (
    <div>
      <ProTable<DataType, Params, ValueType> request={request} {...initTableProps} />
